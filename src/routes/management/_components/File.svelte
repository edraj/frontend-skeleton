<script lang="ts">
  ...
  import { request, RequestType } from "../../../dmart";
  import { _ } from "../../../i18n";
  ...
  async function deleteEntry() {
    if (confirm(`Are you sure you want to delete "${data.displayname}" under ${data.subpath}?`)) {
      let result = await request({request_type: RequestType.delete, space_name: "", records:[{shortname:data.shortname, subpath: data.subpath, resource_type:data.resource_type, attributes:{}}]});
